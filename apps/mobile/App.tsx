...
export default function App() {
  ...
  async function addFood() {
    ...
    const entry: FoodEntry = {
      id: editingFoodId ?? `${Date.now()}`, date: selectedDate, meal: foodForm.meal, name: foodForm.name.trim(), quantity, unit: foodForm.unit.trim() || "g",
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="dark" /><ScrollView contentContainerStyle={styles.content}>
    ...
    {screen === "log" ? <LogScreen foodForm={foodForm} setFoodForm={setFoodForm} scannedFood={scannedFood} addFood={addFood} editingFoodId={editingFoodId} cancelFoodEdit={cancelFoodEdit} query={query} setQuery={setQuery} foodOptions={foodOptions} searching={searching} foodSearchDone={foodSearchDone} doSearch={doSearch} applyApiFood={applyApiFood} savedMeals={savedMeals} savedMealName={savedMealName} setSavedMealName={setSavedMealName} savedMealBase={savedMealBase} setSavedMealBase={setSavedMealBase} createSavedMealFromDay={createSavedMealFromDay} applySavedMeal={applySavedMeal} deleteSavedMeal={deleteSavedMeal} /> : null}
