...
type Screen = "today" | "log" | "scanner" | "fasting" | "progress" | "profile";
...
export default function App() {
  ...
  async function loadCloudData() {
    ...
    try {
      await ensureProfile();
      const [diary, water, exercise, weight, fasting, savedMealRows, nutritionProfile] = await Promise.all([
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="dark" /><ScrollView style={styles.scrollArea} contentContainerStyle={styles.content}>
    ...
    {screen === "today" ? <TodayScreen totals={totals} targets={targets} remaining={remaining} waterTotal={waterTotal} exerciseTotal={exerciseTotal} latestWeight={latestWeight} entries={dayFoods} activeFast={activeFast} setScreen={setScreen} openScanner={openScanner} removeFood={removeFood} startEditFood={startEditFood} /> : null}
...
function TodayScreen({ totals, targets, remaining, waterTotal, exerciseTotal, latestWeight, entries, activeFast, setScreen, openScanner, removeFood, startEditFood }: { totals: { calories: number; protein: number; carbs: number; fat: number }; targets: ReturnType<typeof calculateTargets>; remaining: number; waterTotal: number; exerciseTotal: number; latestWeight?: WeightEntry; entries: FoodEntry[]; activeFast?: FastingSession; setScreen: (screen: Screen) => void; openScanner: () => void; removeFood: (id: string) => void; startEditFood: (entry: FoodEntry) => void }) {
...
function ProfileScreen({ profile, setProfile, targets, saveNutritionProfile, profileSaveMessage, syncing }: { profile: UserProfile; setProfile: Dispatch<SetStateAction<UserProfile>>; targets: ReturnType<typeof calculateTargets>; saveNutritionProfile: () => void; profileSaveMessage: string; syncing: boolean }) {
